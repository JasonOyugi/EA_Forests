...
export const metricMeta: Record<MetricKey, MetricMeta> = {
  ...
  expectedVolume: {
    ...
    unit: "m3",
    format: (value) => `${formatDecimal(value)} m3`,
    axisTick: (value) => compactNumber(value, 2),
  },
  portfolioValue: {
...
export function ChartAreaInteractive({
  ...
  const transitionIndex = chartData.findIndex((point) => point.label === transitionLabel)
  const baselinePoint = chartData.find((point) => point.label === baselineLabel)
  const transitionPoint = chartData.find((point) => point.label === transitionLabel)
  ...
  return (
    ...
    >
      <Card className={`@container/card rounded-xl ${dashboardSurfaceClass}`}>
        <CardHeader>
          <div>
        ...
        <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
          <div className="relative">
            ...
            <ChartContainer
              config={chartConfig}
              className={`aspect-auto h-[250px] w-full text-foreground ${
                isMobile ? "h-[220px]" : ""
              }`}
            >
              <BarChart
                data={tintedChartData}
                margin={{
                  left: isMobile ? 0 : 8,
                  right: 8,
                  bottom: isMobile ? 0 : 4,
              ...
              >
                ...
                <XAxis
                  ...
                  tickFormatter={(value) => formatTrimesterTick(String(value), isMobile)}
